...
from mhs.scaffolding.create_action.command import CreateActionCommand
...
def test_create_action_handles_existing_files_gracefully():
  action_name 
  capability_path = Path(BASE_DOMAIN) / "scaffolding"
  action_path = capability_path / "test_action"
  ...
  req = CreateActionCommand(action_name="Test Action", domain_path=domain_path)
  resp = handle(req)
...
# capability_path = Path(BASE_DOMAIN) / "example" / f"x{uuid.uuid4().hex}"
#   domain_dir = ROOT_DIR / BASE_DOMAIN / "example" / f"x{uuid.uuid4().hex}"
